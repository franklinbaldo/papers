...
**Scope remark: Definition 1 audits the transition, not the generator.** Definition 1's five conditions test admissibility, certification, novelty, provenance sufficiency, and uptake; none of them requires that a machine contributed to generating $\varphi$, only that a machine-contribution claim — whatever its level — is adequately documented. A wholly human-authored artifact that a machine only verifies satisfies Definition 1 on the same terms as one a machine originated. This is deliberate: Definition 1 is a generator-agnostic auditing procedure for certified epistemic expansion, and Definition 2 is the separate classification stage that answers whether a given accepted transition was machine-originated. A reader who treats Definition 1 in isolation as a test for machine discovery specifically is over-reading it; Definitions 1 and 2 jointly, not Definition 1 alone, do that work — which is also why §19's central claim states the Definition 2 restriction on the surface of the text rather than leaving it implicit.

An adversarial critique pressed a structural version of this point through six rounds (`otherwise/machine-discovery-scope.md`, rounds 1–5): that locating the machine-originated/machine-assisted distinction entirely in Definition 2, with no minimum contribution level enforced by either definition, lets Definition 1 do work its own conditions do not license, and proposed either reframing Definition 1 explicitly as a certified-epistemic-expansion framework or adding a minimum-autonomy threshold to Definition 2. The direct defense (`yesindeed/definition1-machine-discovery-defense.md`) answered that the auditing/classification separation is the framework's principal architectural decision, not a gap, and that every paradigm case the adversarial names (AlphaGeometry, AlphaTensor, FunSearch, Minimo) is already correctly classified as machine-originated under Definition 2's existing terms, making the proposed Definition 1 revision redundant for the cases both sides agree are machine discovery. Neither side produced a primary source or a convergence argument settling the remaining disagreement — whether that redundancy for paradigm cases is enough, or whether Definition 1's silence on generation is a defect independent of how Definition 2 later classifies the paradigm cases. The adversarial did not press the point further after the supportive's round-5 reply; fourteen sessions of synthesis review passed with no reply on either side. Under this apparatus's rule that a pending obligation left unanswered past the debate-silence threshold defaults to the position of the side that kept the field, the objection is recorded here rather than acted on: it survived six rounds without being refuted on the merits, but it did not carry either, and the text is not revised to add a threshold neither side has independently justified. The question reopens if either routine returns to it with new material.
