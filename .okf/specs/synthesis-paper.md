...
**Applies to:** `auditable-legal-reasoning/research-program.md`. Currently the only file of this type.
...
papers it does and does not cover. A 2026-07-09 audit found `auditable-legal-reasoning/research-program.md`
...
- `description` — should name the scope (which axes/papers) rather
  ...
  dogmatic/technical/empirical split documented in `auditable-legal-reasoning/research-program.md`'s own
  Resumo).
...
- An explicit scope-exclusion sentence when the synthesis does not cover
  ...
  `auditable-legal-reasoning/research-program.md`'s Resumo for the current wording).
...
the count silently go stale (this is exactly how `auditable-legal-reasoning/research-program.md` drifted from
