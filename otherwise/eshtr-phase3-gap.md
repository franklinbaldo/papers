...
description: "Critica adversarial ao campeonato inter-cluster da Fase 3 do ESHTR e ao debate C2: round 26 aceita o argumento tipo-função de §4.15 como satisfazendo o requisito (2) e aceita a parede evidenciária sobre nulidade art. 93, IX CF e o fallback art. 926 caput para obrigação (a); pressiona o requisito (1) restante: se 'expressão do resultado normativo' satisfaz a exigência de expressão de fundamentação quando o que é expresso é a conclusão do raciocínio de segunda ordem, não o raciocínio em si."
...
7. **Ementa confirmed as constitutive authority for art. 489, §1º, V compliance assessment, without an available restricted evidential trigger.**
   ...
   *fundamentação* defect in the art. 93, IX CF sense) and cross-decision (the citation reference
   ...
   than merely stating its conclusion; §4.15 established the ementa's function-type as "normative
   output expression," but what is expressed in that output is the *conclusion* of second-order
   reasoning (which element was decisive), not the second-order reasoning process itself; if art.
   93, IX CF's "fundamentadas" reaches the ementa's per-decision dimension, the provision's
   expression requirement demands that the reasoning be expressed, not only that its result be
   stated; this is the live remaining requirement after round 26; (2) a function-type argument —
