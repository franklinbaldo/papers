...
description: "Critica adversarial ao campeonato inter-cluster da Fase 3 do ESHTR e ao debate C2 sobre o escopo do art. 93, IX CF para a ementa: round 25 aceita a separação de nível de ator de r24 (iv-b retirado) e contesta o critério de caráter de ato com o paralelo do relatório (iv-a) e a lacuna de fundamentação doutrinária para nulidade por inconsistência interna do acórdão (iv-c)."
...
7. **Ementa confirmed as constitutive authority for art. 489, §1º, V compliance assessment, without an available restricted evidential trigger.**
   ...
   *fundamentação* defect in the art. 93, IX CF sense) and cross-decision (the citation reference
   ...
   nullity. Path (b) now additionally requires the supportive to show all of the following:
   (1) that the per-decision dimension of the ratio-constitutive function constitutes reasoning-
   expression in art. 93, IX CF's sense — specifically, that designating which element of already-
   expressed reasoning was decisive constitutes expressing reasoning rather than indexing it, such
   that the accountability function art. 93, IX CF serves is impaired by the elevation error
   despite the votos' reasoning remaining accessible; (2) a function-type argument — not merely a
   location-in-process argument — showing that the ementa-authorship act's function falls within
   art. 93, IX CF's reasoning-expression domain rather than arts. 926–927's citation-coherence
   domain, accounting for the *relatório* parallel: since both the ementa and the *relatório* are
   relator-authored formal acórdão components under art. 943 embedded in the same authorship
   process, the argument must distinguish them on grounds internal to functional-domain assignment
   under the round-20 accepted functional-differentiation framework, not merely cognitive character
   or deliberative complexity; (3) that the accuracy structure's implicit evidential presuppositions
   are coherent with the constitutive account maintained for §1º, V compliance purposes, or that
   the two accounts operate at structurally distinct levels — this requirement is substantially met
   by the round-25 accepted actor-level separation; the remaining open question is what provision
   governs obligation (a)'s accuracy requirement at the cited court level if art. 93, IX CF's
   function-type basis is not established from (2); and (4) doctrinal grounding for acórdão-level
   internal-consistency nullity — STF precedent or authoritative Brazilian procedural doctrine
   establishing that art. 93, IX CF nullity attaches to ementa-voto inconsistency as a distinct
   defect category, separable from and not reducible to voto reasoning-expression failure; without
   this grounding, the provision's traditional scope — nullity for reasoning-expression failure in
   the votos — governs, and the simplified (a)/(b) structure grounding obligation (a) in art. 926
   caput's per-event integrity requirement and obligation (b) in arts. 926–927's compliance
   framework without art. 93, IX CF remains the adversarial's structural claim. Under
