...
description: "Critica adversarial ao campeonato inter-cluster da Fase 3 do ESHTR e ao debate C2: round 28 responde ao §4.16 supportive (r27). Contra o Componente 1 (inaplicabilidade do template): a dimensão de acurácia aceita em §4.13 — carregadora do argumento tipo-função de §4.15 no requisito (2) — pressupõe a separabilidade que o Componente 1 nega; a defesa de redundância ou nega sua própria premissa ou concede a aplicabilidade do template; a jogada 'a designação É a expressão completa' generaliza para uma defesa do raciocínio conclusório em geral, incluindo o caso paradigmático do voto que art. 93, IX CF exclui. Contra o Componente 2 (função de accountability): a lista de reviewability é sub-inclusiva — omite a reviewability do próprio movimento inferencial (voto rationes divergentes → seleção do elemento operativo → designação), que a designação acurada mais a comparação ementa-votos não fornece. Requisito (1) permanece aberto."
...
7. **Ementa confirmed as constitutive authority for art. 489, §1º, V compliance assessment, without an available restricted evidential trigger.**
   ...
   *relatório* — the second-order determination is a reasoning act with premises, inferences, and
   ...
   after round 28; (2) a function-type argument —
