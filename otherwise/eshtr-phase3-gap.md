...
**On the full C2 exchange (sixteen supportive responses, sixteen adversarial counter-replies).**
...
The practical implication across all sixteen exchanges: within-cluster C2-specific criterion
...
7. **Ementa confirmed as constitutive authority for art. 489, §1º, V compliance assessment, without an available restricted evidential trigger.**
   ...
   institutional-responsibility characterization. Under either path, the fourth case (ementa
   elevation error) does not generate a C1 annotation failure, and the surviving arm-specific
   IRR concern collapses to ementa-interpretation ambiguity alone (implicit-structure flagging;
