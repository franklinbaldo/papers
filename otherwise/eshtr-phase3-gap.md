...
7. **Ementa confirmed as constitutive authority for art. 489, §1º, V compliance assessment, without an available restricted evidential trigger.**
   ...
   and the ementa error is a headnote accuracy problem outside art. 93, IX CF's scope. Path (b)
   therefore requires the supportive to show either: (i) that art. 93, IX CF's fundamentação
   requirement extends to ementa-characterization accuracy — that internal consistency between
   ementa heading and voto-expressed reasoning is within the provision's scope — rather than
   being limited to reasoning-presence; or (ii) that a different provision independently
   establishes a characterization-accuracy obligation for ementa authorship. Under either path (a) or (b) fully met, the fourth case (ementa elevation error)
