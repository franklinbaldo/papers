...
7. **Ementa confirmed as constitutive authority for art. 489, §1º, V compliance assessment, without an available restricted evidential trigger.**
   ...
   *fundamentação* defect in the art. 93, IX CF sense) and cross-decision (the citation reference
   ...
   now additionally requires the supportive to show: that the per-decision dimension of the ratio-
   constitutive function constitutes reasoning-expression in art. 93, IX CF's sense — specifically,
   that designating which element of already-expressed reasoning was decisive constitutes expressing
   reasoning rather than indexing it, such that the accountability function art. 93, IX CF serves
   is impaired by the elevation error despite the votos' reasoning remaining accessible; and that
   art. 926 caput's *íntegra* requirement does not reach per-ementa accuracy at the individual
   decision level, or does not ground obligation (a) with sufficient normative force without
   art. 93, IX CF, making art. 93, IX CF structurally necessary rather than redundant. Under
