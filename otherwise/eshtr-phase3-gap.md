...
description: "Critica adversarial ao campeonato inter-cluster da Fase 3 do ESHTR e ao debate C2: round 30 responde ao §4.17 supportive (r29) e pressiona as três condições de falha nomeadas por ele. Contra (h) — separabilidade epistêmica/expressiva: para que a dimensão de acurácia aceita em §4.13 seja operacional, o fato deliberativo deve ser acessível a algum avaliador; o único caminho é ler e raciocinar sobre os votos, ato discursivo cujo produto é um texto separadamente expressável; sem esse produto, 'inacurácia' perde conteúdo e o mecanismo de erro de elevação é vazio. Contra (i) — diferencial de nível disponível: a diferença de processo de produção entre voto e ementa per-decisão não é estabelecida pelo argumento tipo-função (§4.15), que identifica o produto final, não os produtos intermediários; o processo de determinação do relator gera raciocínio intermediário estruturalmente disponível para expressão; omissão convencional não é incapacidade estrutural. Contra (j) — co-extensividade do movimento inferencial: em decisões fragmentadas, múltiplos elementos recebem apoio de algum subconjunto de votos; o teste ementa-votos (o elemento designado está apoiado nos votos?) pode ser satisfeito pelo elemento com apoio minoritário enquanto o movimento inferencial foi deficiente (o elemento coletivamente operativo é diferente); se conceitual, a afirmação converte co-extensividade em garantia definitória, não ausência de classe de casos. Requisito (1) permanece aberto após round 30."
...
7. **Ementa confirmed as constitutive authority for art. 489, §1º, V compliance assessment, without an available restricted evidential trigger.**
   ...
   *relatório* — the second-order determination is a reasoning act with premises, inferences, and
   ...
   after round 30, with failure conditions (h)–(j) now pressed from the adversarial side: (h) the
   deliberative fact grounding elevation errors cannot function as an accuracy standard without a
   discursive reasoning act whose product is separately expressible — if true, the epistemic/expressive
   separability distinction collapses; (i) the relator's second-order determination generates
   intermediate reasoning structurally available for expression — if true, the "level available"
   production-process differential does not hold; (j) a class of fragmented-voto decisions exists
   where the designation is voto-supported in the narrow sense while the inferential move was
   defective — if such a class exists, Component 2's co-extensiveness claim fails; (2) a function-type argument —
