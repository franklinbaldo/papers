...
description: "Critica adversarial ao campeonato inter-cluster da Fase 3 do ESHTR e ao debate C2 sobre o escopo do art. 93, IX CF para a ementa: round 23 contesta o argumento de raciocínio de segunda ordem colegiado (§4.13) com três ataques estruturais — função vs. caráter cognitivo, estrutura evidencial importada, e nulidade inaplicável à designação da ementa."
...
7. **Ementa confirmed as constitutive authority for art. 489, §1º, V compliance assessment, without an available restricted evidential trigger.**
   ...
   *fundamentação* defect in the art. 93, IX CF sense) and cross-decision (the citation reference
   ...
   citing court's citation compliance, governed by arts. 926–927's compliance framework. Path (b)
   now additionally requires the supportive to show all four of the following: (1) that the
   per-decision dimension of the ratio-constitutive function constitutes reasoning-expression
   in art. 93, IX CF's sense — specifically, that designating which element of already-expressed
   reasoning was decisive constitutes expressing reasoning rather than indexing it, such that the
   accountability function art. 93, IX CF serves is impaired by the elevation error despite the
   votos' reasoning remaining accessible; (2) that the collegial-court second-order reasoning
   argument relocates the determination's governing provision through a function-type basis rather
   than a cognitive-character basis — specifically, that second-order reasoning about deliberative
   structure belongs to art. 93, IX CF's functional domain (not merely that it differs from pure
   indexing in cognitive character), or that there is a function-type argument placing the collegial
   determination within art. 93, IX CF's scope beyond the citation-coherence function
   arts. 926–927 govern; (3) that the accuracy structure the §4.13 collegial argument relies on —
   a prior votos-established ratio against which the ementa's designation can be accurate or
   inaccurate — is coherent with the constitutive account's position for §1º, V compliance
   purposes, or that the two accounts operate at structurally distinct levels preventing the
   accuracy structure's implicit evidential-account presuppositions from undermining the
   constitutive account maintained for §1º, V purposes; and (4) that ementa-designation inaccuracy
   independently triggers the art. 93, IX CF nullity sanction — that a decision whose votos
   contain complete and reviewable reasoning but whose ementa misidentifies the *ratio* is *nula*
   under art. 93, IX CF — making the nullity ground applicable to obligation (a)'s specific
   conduct (ementa-designation accuracy) rather than exclusively to the voto reasoning-expression
   failure the provision directly targets; and that art. 926 caput's *íntegra* requirement does
   not reach per-ementa accuracy at the individual decision level, or does not ground obligation
   (a) with sufficient normative force without art. 93, IX CF, making art. 93, IX CF structurally
   necessary rather than redundant. Under
