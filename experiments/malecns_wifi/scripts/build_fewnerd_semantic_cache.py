"""Stage A: encode Few-NERD tokens once with frozen MiniLM/E5 (word-pooled).
...
def main() -> None:
    ...
    telemetry.summary({
        "documents": manifest["sentences"], "tokens": manifest["tokens"], "bytes": manifest.get("bytes"),
        "fingerprint": manifest.get("fingerprint"),
        **{f"{e['slug']}/encode_seconds": e["encode_seconds"] for e in manifest["encoders"]},
        **{f"{e['slug']}/tokens_per_second": e["tokens_per_second"] for e in manifest["encoders"]},
    })
    ...
    if args.push_to_hub:
        ...
        from malecns_wifi.fewnerd_cache import push_to_hub
