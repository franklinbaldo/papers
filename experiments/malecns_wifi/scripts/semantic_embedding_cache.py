...
The cache preserves the exact byte-aligned semantic fields used by the MaleCNS
experiments while allowing the expensive frozen SentenceTransformer forward pass
to happen once.  Training code then consumes only numpy arrays from disk.
"""
...
def _make_model(model_name: str, backend: str, device: str):
    ...
    if backend == "openvino":
        kwargs = {"backend": "openvino", "model_kwargs": {"device": device.upper()}}
...
def build_cache(*, model_names, examples, scales, output: Path, backend: str = "openvino",
                device: str = "GPU", batch_size: int = 64):
...
def load_cache(*, model_names, examples, scales, cache: Path):
    ...
    if manifest.get("fingerprint") != expected:
        raise RuntimeError("embedding cache fingerprint mismatch; rebuild the cache for this corpus/models/scales")
