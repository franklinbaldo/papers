...
def _channel(pyramid, scale: int, encoder: str = "enc", revision: str = "r1"):
    return pyramid[(scale, encoder, revision)]
...
def test_pyramid_uses_overlapping_half_stride_tiles() -> None:
    ...
    assert [(t.start, t.end) for t in _channel(pyramid, 8)] == [
        (0, 8), (4, 12), (8, 16), (12, 20)
    ]
    assert _channel(pyramid, 4)[1].start == 2
    assert _channel(pyramid, 4)[1].end == 6
...
def test_same_scale_can_have_two_encoders_without_fake_cross_relation() -> None:
    tokens = [f"t{i}" for i in range(16)]
    plan = PyramidPlan(
        (
            _spec(16, "jina"),
    ...
    pyramid = build_pyramid(tokens, plan)
    pairs = relation_pairs(pyramid)

    assert len(_channel(pyramid, 8, "jina")) == len(_channel(pyramid, 8, "mini"))
    assert all(child.spec.encoder_id == parent.spec.encoder_id for child, parent in pairs)
    assert {child.spec.encoder_id for child, _ in pairs} == {"jina", "mini"}
...
def test_redundant_ancestors_are_allowed() -> None:
    ...
    child0_parents = [
        ...
        if child.spec.scale == 8 and child.start == 0
    ]
