...
```
   ...
   Neuropilar Block          Exponential Decay         Duplication-Divergence    Directed SVD Kernel
   Affinity Matrix           $P(u,v) \sim e^{-d/\lambda}$  (Bilateral Chiasm)    (Modal Gain Synthesis)
```

### 4.1 Model 1: Neuropilar Stochastic Block Model (SBM)
The canonical anatomical taxonomy of the adult *Drosophila* brain defines 78 neuropils (e.g., Antennal Lobe, Mushroom Body Calyx, Central Complex, Optic Glomeruli). In the compiled MaleCNS v1.0 subvolume artifact (`graph.npz`) used throughout this work, exactly **58 neuropilar compartments** contain traced synaptic arborizations and somatic centroids.
- Let $\mathbf{M} \in \mathbb{R}^{K \times K}$ (where $K = 58$ for the empirical subvolume and $K = 78$ for full-brain taxonomy) be the coarse-grained neuropilar traffic matrix, where $M_{AB}$ is the probability of a synapse from a neuron in neuropil $A$ to neuropil $B$.
...
### 6.5 The Post-Evolutionary Connectome: Pareto Superiority Under Biological Identity Constraints
The ultimate ambition of procedural synthesis transcends merely asking whether a biological connectome is specialized for sensorimotor steering and bottlenecked for generic computation. The fundamental scientific question is whether a procedurally synthesized connectome can become **functionally superior to the biological reference model**:

> **Can a procedurally synthesized connectome outperform the biological MaleCNS as a fly while remaining admissibly fly-like in its internal dynamics?**

Rather than relying on unconstrained behavioral optimization (which risks discovering an alien controller that solves FlyDoom without preserving neural plausibility), we operationalize the "post-evolutionary fly" as **Pareto superiority under explicit biological-identity constraints**:

1. **Pareto Dominance Relation ($\succ_P$):**
   A synthetic graph $\mathcal{G}_a$ Pareto-dominates $\mathcal{G}_b$ ($\mathcal{G}_a \succ_P \mathcal{G}_b$) if:
   $$\mathcal{F}_i(\mathcal{G}_a) \ge \mathcal{F}_i(\mathcal{G}_b) \quad \forall i \in \{1, \dots, M\} \quad \text{and} \quad \exists j: \mathcal{F}_j(\mathcal{G}_a) > \mathcal{F}_j(\mathcal{G}_b)$$
   across all pre-registered behavioral objectives $\mathcal{F}$ (e.g., closed-loop evasion rate, food acquisition speed, sensory noise robustness, lesion recovery, and novel arena generalization).

2. **Biological Identity Admissibility Region ($\mathcal{I}_{\text{bio}}$):**
   A candidate synthetic graph $\mathcal{G}^* = \mathcal{P}(\boldsymbol{\theta})$ must remain strictly inside the pre-registered biological identity manifold $\mathcal{I}_{\text{bio}}(\mathcal{G}^*, \mathcal{G}_{\text{bio}}) \ge \tau$, bounded by:
   - Analytical Markov state transition operator divergence: $\mathcal{D}_{\text{Markov}}(\mathcal{G}^*, \mathcal{G}_{\text{bio}}) \le \epsilon_M$;
   - Continuous latent manifold alignment: $\text{CKA}(\mathbf{X}^*, \mathbf{X}_{\text{bio}}) \ge \tau_{\text{CKA}}$;
   - Mesoscopic neuropilar routing consistency: $\|\widehat{\mathbf{M}}^* - \widehat{\mathbf{M}}_{\text{bio}}\|_F \le \epsilon_{\text{meso}}$.

3. **Wiring Economy & Computational Efficiency:**
   The candidate graph must satisfy a structural parsimony criterion: $|E^*| < |E_{\text{bio}}|$ (fewer total synapses and lower metabolic cost for equivalent or superior behavioral throughput).

Under this formalization, the post-evolutionary connectome is not an idle metaphor, but an **operational, testable hypothesis**:
$$\boxed{\exists \;\boldsymbol{\theta}^*: \mathcal{P}(\boldsymbol{\theta}^*) \succ_P \mathcal{G}_{\text{bio}} \quad \text{subject to} \quad \mathcal{I}_{\text{bio}}\big(\mathcal{P}(\boldsymbol{\theta}^*)\big) \ge \tau}$$
A positive result provides empirical evidence that procedural generation can navigate developmental trade-offs and discover circuit configurations that surpass natural evolutionary baselines within defined operational environments.
...
- **Protocol:**
  1. Compute the empirical inter-neuropil transition probability matrix $\mathbf{M} \in [0, 1]^{K \times K}$ (with $K = 58$ for the compiled graph subvolume and $K = 78$ for full-brain taxonomy) from `graph.npz`.
  2. Sample synthetic adjacency graphs $\mathcal{G}_{\text{SBM}}$ conditioned on $\mathbf{M}$ and marginal in/out-degree sequences. We distinguish between *connectivity matrix compression* (~1,640×–2,970× reduction: $58 \times 58 = 3,364$ or $78 \times 78 = 6,084$ inter-neuropil weights vs. $10^7$ edges) and *total generative state description* (~30× reduction if preserving explicit per-neuron degree sequences; >1,000× if nodal degrees are drawn from neuropil-specific parametric distributions).
