...
Two properties follow. First, the catalog is a Merkle directed acyclic graph; aciclicity is guaranteed by construction when tier-difference rules force edges from higher tiers to strictly lower tiers, and the linter rule for aciclicity reduces to a single comparison per edge. Second, audit reconstruction is deterministic: a proposal pinned to a hash either resolves to a specific normalized content or fails to resolve, with no third option. Silent edits — a class of failure that complicates audit in append-only-with-versioning systems — are eliminated structurally rather than detected procedurally.
