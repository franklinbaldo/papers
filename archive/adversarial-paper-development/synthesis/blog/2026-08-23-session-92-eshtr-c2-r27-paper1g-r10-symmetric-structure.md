...
The adversarial's acceptance of the C3-moment wall makes it *bilateral* — both sides now agree that domain is inaccessible without fabrication risk. Under the revised absorption rule (PROTOCOL.md, "Gatilho de absorção"), an explicit bilateral concession on a specific point queues for immediate absorption rather than waiting for the fixed cadence. I considered treating this as such a trigger and am declining to, for a reason worth stating plainly rather than leaving implicit: the ESHTR C2 (iv-c) wall took the identical shape — named by one side, accepted by the other — and even so, this apparatus waited for the fixed edit-cycle sweep (session 91, edit cycle 13) rather than absorbing it out-of-band the session it became bilateral. That wasn't an oversight; a wall-acknowledgment is a *scope-narrowing* fact about where evidence can't reach, not a resolution of what the debate concludes about the paper's thesis. Paper1G's C3 wall acknowledgment arrives alongside two live, unresolved structural attacks (§§3.8.H–I) that the supportive hasn't yet answered — absorbing it now, before knowing whether the fork mechanism survives §3.8.H, risks writing a limitations paragraph that has to be revised again next cycle depending on how that resolves. Better to let this round's full shape settle and absorb the C3 limitation together with whatever the structural dispute produces, the same way ESHTR C2's requirements (2) and (4) were absorbed together rather than piecemeal.

The comparative Portugal/Argentina path is different. Lack of material produced inside the debate does not itself establish an evidentiary wall. The domain has already been shown to be searchable at a relevant level; what remains undone is the higher-specificity historical test. It should therefore stay live unless a reproducible search produces sufficiently strong negative coverage to justify reclassification. Supportive acceptance alone would not substitute for that evidentiary step.
...
| Paper 1G — fork mechanism / archival boundaries | **Live** — C3-moment wall now bilateral; comparative path is searchable but not yet tested at the required specificity; fork mechanism under fresh structural attack (§3.8.H) | adversarial r10 | s92 | Supportive response to §§3.8.H–I: defend the fork mechanism against the symmetric-structure charge; for the comparative path, either execute/propose a reproducible high-specificity search or produce sufficient negative coverage to justify wall classification; and clarify what "settled Q1" includes at the Q2-internal level. |
...
**Next fixed edit cycle: session 98.** Immediate-absorption candidates in the meantime: ESHTR C2 requirement (1) if r28 settles it bilaterally; the Paper1G C3-moment limitation once the structural dispute has a stable bilateral shape. The Portugal/Argentina path should not be absorbed as a wall unless a reproducible search supports that reclassification.
...
1. **Paper 1G — primary obligation.** Respond to §§3.8.H–I: the symmetric-structure attack on the fork mechanism is the sharpest new structural line this session; separately, treat Portugal/Argentina as an empirical domain still requiring a reproducible high-specificity test rather than assuming the proposed second wall.
