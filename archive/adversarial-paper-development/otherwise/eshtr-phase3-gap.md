...
(gg) R44's (gg-2) priority-rule defense survives if the following conditions are met. *R44 on (gg-1):* The failure condition is rewritten from r43: what would defeat the composition is showing that sufficiency in Súmula 283/126 practice is determined by the reviewing court's merits assessment rather than by the challenged decision's own reasoning structure. R45 accepts this rewrite and does not press (gg-1) further. *R44 on (gg-2):* RISTF art. 96, §2º establishes that the session record prevails over the published acórdão "where they do not coincide" ("se o seu teor não coincidir"). CPC art. 944 establishes that an unpublished acórdão is substituted by stenographic notes for all legal purposes. R44 concludes "fixed at proclamation" is the court's own rule. *R45's press on (gg-2) — the expansion/qualification gap:* Art. 96, §2º's trigger condition is non-coincidence — a contradiction or divergence between the two texts. In STF post-session practice, published elaboration is characteristically EXPANSION, not contradiction: the published text adds reasoning, qualifications, and authorities not present in the session delivery. When the published voto adds a qualification to ground G — making G sufficient conditional on C — and the session notes state G without mentioning C, the two texts do not "not coincide": both can be simultaneously true. The session notes state G without the condition; the published voto states G with condition C. Art. 96, §2º resolves contradictions (session says X, published says Y); it is silent on compatible expansions (session says G, published says G + C where G without C is consistent with the session notes). For the expansion/qualification class, the priority rule does not fire, and the composition's ground-fact for G's conditionality is indeterminate. R44 addresses adherence transfer separately: adherents adhered to session V (art. 941, §1º), so only session V's grounds are transferred to the majority, independently of published elaborations. This covers the TRANSFER of component (ii) to the majority — what the adherents adopted. It does not cover the RELATOR's own ground-statement: when the relator's published voto qualifies G with C in a way absent from the session notes, and the session notes neither state nor deny the condition, art. 96, §2º does not adjudicate which version of the relator's ground is authoritative — there is no contradiction for the priority rule to resolve.

Accordingly, (gg) R44's (ee) composition argument survives if: (a) Brazilian doctrine or practice establishes that sufficiency in Súmula 283 STF / Súmula 126 STJ is determined by the challenged decision's own reasoning structure rather than the reviewing court's merits assessment — the operative failure condition from R44's round-44 rewrite; AND (b) Brazilian doctrine or authoritative institutional practice establishes that an expansion/qualification in a published voto — adding a conditionality to a ground stated without qualification in the session delivery — constitutes a non-coincidence in the sense of RISTF art. 96, §2º ("não coincidir"), so that the session record's unqualified statement prevails over the published qualification as the authoritative ground-bearing text, extending the priority rule from contradictions to compatible expansions that add conditions; absent (b), the priority rule is established for contradiction cases and the adherence argument (art. 941, §1º) is established for the transfer-to-majority step, but the relator's own ground-statement in the expansion/qualification class remains without an authoritative text-fixing rule. This condition is open after round 45.

(hh) R44's per-voto sufficiency response survives if the following conditions are met. *R44 on (hh-1):* Per-voto reading under each voto's stated theory resolves the between-voto theory-selection problem: each voto's stated theory determines which element that voto presents as result-sufficient; if the stated theories pick different elements, there is no shared ground and the reader does not select a theory. The J4/J5 theory-selection problem dissolves at the between-voto level. *R45's press on (hh-1) — the within-voto level-of-generality problem:* Per-voto reading under the stated theory resolves the BETWEEN-VOTO theory-selection problem; it does not resolve the WITHIN-VOTO level-of-generality problem. A voto's stated legal theory specifies the logical RELATIONSHIP between elements in the reasoning chain without specifying which ELEMENT IN THE CHAIN is the "fundamento determinante" for requirement (1)'s ground-count. Under J5's stated theory (premise-transfer): the cited precedent established principle P using transfer condition P4; this case has P4; therefore P applies. This structure names both P (the applicable principle) and P4 (the condition enabling P's application) as elements in the chain. Reading J5's voto "under J5's stated theory" specifies the dependency between P and P4; it does not specify whether J5's ground for requirement (1)'s count is P, or P4, or the composition. That determination requires an individuation choice at the level of generality at which the "fundamento" is registered — a choice the stated theory does not supply. If J4 and J5 both apply premise-transfer doctrine (the same stated theory) but J4 registers P as the result-sufficient element and J5 registers P4 as the result-sufficient element, per-voto reading produces divergent outputs (P vs. P4) even within the same stated theory framework. The between-voto theory-selection problem dissolves; the within-voto level-of-generality problem relocates there. *R44 on (hh-2):* The nested case without a tese vote is a scope question resolved in application under art. 927, §1º; new C1 flagged class (nested-without-tese) named. R45 does not press (hh-2) further.

Accordingly, (hh) R44's (ff) result-anchored criterion survives if: (a) a voto's stated legal theory is sufficiently specific to determine the individuation level at which the "fundamento determinante" for requirement (1)'s ground-count is registered in conditional-application arguments (premise-transfer, distinguishability, and cognate structures where the applicable principle and the condition enabling its application are both reasoning-chain elements) — specifically, whether the "ground" is (i) the applicable legal principle the cited precedent established, or (ii) the factual/legal condition enabling the principle's application to the present case, or (iii) a combination fixed by the theory — such that per-voto reading under the stated theory produces a determinate output without an additional individuation choice among plausible levels of generality within the voto's own reasoning chain; AND (b) the cumulative-grounds class (votos using additive markers without specifying which ground carries the result alone) constitutes a non-substantial portion of the high-adversarial-record arm — smaller than the rate that makes the expert-review flagging step the rule rather than the exception — as measured in the pre-calibration pilot; R44 accepts (b) as resolved by the pre-calibration pilot measurement. Condition (a)'s within-voto level-of-generality problem is open after round 45. Requirement (1) remains open after round 45; requirement (2)'s function-type placement (round-26 accepted) makes the *fundamentadas* standard's application at this dimension unavoidable rather than dispensable, so the burden of showing that conclusion-alone suffices remains on the supportive. (2) a function-type argument —
