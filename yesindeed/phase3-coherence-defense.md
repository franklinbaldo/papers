...
The calibration gap is real: the current ESHTR calibration protocol (§5.4) does not include a Phase 3-specific test for whether LLM outputs under the abstraction instruction reflect structural rather than domain-specific criterion application. A targeted calibration component could test this directly: present calibration judges with the same decisions under standard Phase 2 domain-specific evaluation instructions and under the Phase 3 abstraction instruction, then compare whether per-criterion score profiles shift in the direction the instruction specifies. A reliable shift — reduced cross-cluster variance in criterion profiles, increased correlation between Phase 2 intra-cluster ranking and Phase 3 cross-cluster ranking — would confirm that the instruction redirects output criterion application. This is a tractable addition to the existing calibration framework, not a test of a structural impossibility.
...
- If a Phase 3-specific calibration test shows that per-criterion score profiles under the abstraction instruction do not shift relative to Phase 2 domain-specific profiles — i.e., the same criterion emphasis patterns appear regardless of the instruction — the representation/output-mapping distinction in §4.6 fails to hold for the LLM panel used, and the instruction-following defense of Option (c) is empirically disconfirmed.
